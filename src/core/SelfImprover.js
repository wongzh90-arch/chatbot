...
export class SelfImprover {
    ...
    async loadManifest() {
        try {
            ...
            this.onLog('✅ Manifest loaded');
        } catch {
            this.onLog('⚠️ No manifest.json – run /manifest-build first');
        }
    }
    ...
    async _plan(goal) {
        ...
        const context = ContextBuilder.buildContext({ targetContents: [], relatedContents: [], manifest: this.manifest });
        const prompt = `Create a plan for: "${goal}"
Repo: ${this.repo}, branch: ${this.branch}
Key files: ${relevant.join(', ')}
${context.contextString}
Return ONLY JSON: { "milestone_title": "short", "analysis": "sentence", "tasks": [{"title":"..","description":"..","files":["path"]}] }
Max 4 tasks, each ≤2 files.`;

        const reply = await LLMProvider.chatCompletion({
            provider: this.provider, model: this.model, messages: [],
            systemPrompt: 'You are a senior developer. Output only JSON.',
            userContent: prompt, thinkingMode: this.thinkingMode, reasoningEffort: this.reasoningEffort
        });
        try {
    ...
    async _executeTask(task) {
        ...
        for (const p of allPaths) {
            try {
                const { content, sha } = await GitHubService.loadFileContent(this.repo, this.branch, p, this.githubToken);
        ...
        const context = ContextBuilder.buildContext({ targetContents, relatedContents: related, manifest: this.manifest });
...
Output COMPLETE new content for each file inside <skill name="update_editor" file="path">...</skill> blocks.`;
        ...
        if (Object.keys(lintable).length) {
            const [syntax, lint] = await Promise.all([ExecutorAPI.syntax(lintable), ExecutorAPI.lint(lintable)]);
            const errors = [...(syntax.errors || []), ...(lint.errors || [])].filter(e => e.severity !== 'warning');
            if (errors.length) {
        ...
        try {
            await GitHubService.commitMultipleFiles(this.repo, this.branch, fileMap, `Task: ${task.title}`, this.githubToken);
            return true;
        } catch (e) {
    ...
    _initTaskQueue(tasks) {
        ...
        for (const t of tasks) {
            this.taskQueue.tasks.push({
                id: this.taskQueue.nextId++, status: 'TODO', title: t.title,
                description: t.description, files: t.files
            });
        }
    }
    ...
    _markTaskDone(id) { const t = this.taskQueue?.tasks.find(t => t.id === id); if (t) t.status = 'DONE'; }
    _markTaskFailed(id) { const t = this.taskQueue?.tasks.find(t => t.id === id); if (t) t.status = 'FAILED'; }
    _markTaskTodo(id) { const t = this.taskQueue?.tasks.find(t => t.id === id); if (t) t.status = 'TODO'; }
    _markTaskReviewPassed(id) { const t = this.taskQueue?.tasks.find(t => t.id === id); if (t) t.status = 'DONE'; }
}
