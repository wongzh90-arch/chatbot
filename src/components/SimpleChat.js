...
export function SimpleChat({ services }) {
  ...
  const handleSend = async () => {
    ...
    if (cmd === '/self-improve') {
      ...
      if (isRunning) {
        addToast('Already running – use /pause first', 'warning');
        return;
      }
      ...
      try {
        improverRef.current = services.createImprover({
          ...
          onClarificationNeeded: async (questions) => {
            ...
            // Wait for user input (simple prompt – you can replace with a better UI)
    ...
    if (cmd === '/index') {
      // … (unchanged) …
      if (isRunning) {
        addToast('Already running', 'warning');
        return;
      }
      if (!workspace.currentRepo || !workspace.githubToken) {
        addMessage('assistant', '❌ Please fill in the repository and GitHub token fields first.');
    ...
    if (cmd === '/help') {
      addMessage(
        'assistant',
        'Commands: `/index`, `/self-improve "goal"`, `/pause`, `/clear`'
      );
      return;
    }
