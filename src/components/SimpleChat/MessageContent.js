...
export function MessageContent({ content, isUser, fontSize, compact }) {
    ...
    return createElement('div', {
        style: {
            ...
            color: isUser ? '#e5e5e5' : '#d4d4d4'
        }
    }, segments.map((seg, i) => renderSegment(seg, i, compact, baseFontSize)));
}
...
function renderSegment(seg, key, compact, baseFontSize) {
    if (seg.type === 'code') {
        return createElement(CodeBlock, { key, lang: seg.lang, code: seg.content, compact });
    }
    return createElement('div', { key, style: { whiteSpace: 'pre-wrap', wordBreak: 'break-word' } },
        renderTextWithBlocks(seg.content, compact, baseFontSize));
}

function renderTextWithBlocks(text, compact, baseFontSize) {
    const elements = [];
    ...
    while ((m = thinkingRegex.exec(text)) !== null) {
        if (m.index > last) {
            elements.push(...renderTextLines(text.slice(last, m.index), `t${i++}`, compact, baseFontSize));
        }
        elements.push(createElement(ThinkingBlock, { key: `th${i++}`, content: m[1].trim(), compact }));
        last = m.index + m[0].length;
    }

    if (last < text.length) {
        elements.push(...renderTextLines(text.slice(last), `t${i++}`, compact, baseFontSize));
    }
    if (elements.length === 0) {
        elements.push(...renderTextLines(text, `t0`, compact, baseFontSize));
    }
    return elements;
}

function renderTextLines(text, keyPrefix, compact, baseFontSize) {
    ...
    const flushText = (k) => {
        if (currentText) {
            elements.push(createElement('span', { key: `${keyPrefix}-line-${k}` }, renderInlineCode(currentText, baseFontSize)));
            currentText = '';
        }
    };
...
function ThinkingBlock({ content, compact }) {
...
function CodeBlock({ lang, code, compact }) {
    ...
    return createElement('div', {
        ...
        createElement('div', {
            ...
            createElement('span', { style: { fontSize: 10, color: '#888', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: 0.5 } }, lang || 'text'),
            createElement('button', {
                onClick: handleCopy,
                style: {
                    background: 'none',
...
function renderInlineCode(text, baseFontSize) {
